...
public class QuestionsActivity extends AppCompatActivity {
    ...
    private void getQuestion() {
            ...
            for (int j = 0; j < idQuestions.size(); j++) {
                for (int k = 0; k < selectedQuestions.size(); k++) {
                    if (idQuestions.get(j).toString().equals(selectedQuestions.get(k))) {
                        ...
                        if (j > 0) {
                            j--; //TODO: Sprawdzić czy jest dobrze ta linijka.
                        }
                    }
                }
            }
        ...
        positions.add(idQuestions.get(position).toString());
        selectedQuestions.add(idQuestions.get(position).toString());
    }
    ...
    private void sendNotificationToPlayer() {
        FCMNotificationSend FCMNotificationSend = new FCMNotificationSend(friendToken,
                getResources().getString(R.string.your_turn_notification), getApplicationContext(), QuestionsActivity.this);
